-- Add settlement reversal counts to the manager revenue report.

create index if not exists order_settlement_reversals_restaurant_reversed_at_idx
  on public.order_settlement_reversals (restaurant_id, reversed_at);
...
begin
  ...
  return (
    ...
    reversal_summary as (
      select count(*)::integer as reversal_count
      from public.order_settlement_reversals
      where restaurant_id = target_restaurant_id
        and reversed_at >= range_start
        and reversed_at < range_end
    ),
    dish_sales as (
      select
    ...
    select jsonb_build_object(
      'summary', jsonb_build_object(
        ...
        'reversalCount', reversal_summary.reversal_count
      ),
      'dishSales', coalesce((
    ...
    from summary
    cross join reversal_summary
  );
end;
$$;

revoke all on function public.get_revenue_report(text, timestamptz, timestamptz) from public, anon;
