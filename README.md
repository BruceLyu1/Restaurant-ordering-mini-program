...
- 后台已结账订单支持按订单号、订单序号、桌号及结账日期范围即时查询，便于快速核对历史订单。
- 经理撤销结账严格限定为 `Asia/Hong_Kong` 当天完成的结账；前端、本地演示模式与 Supabase RPC 均执行同一限制。跨日订单保留结账及审计记录，但不可撤销。
- Supabase 项目需要按顺序执行 `supabase/migrations/20260702019000_same_day_settlement_reversal.sql`，数据库端才会同步启用跨日撤销保护。
## 在线访问
